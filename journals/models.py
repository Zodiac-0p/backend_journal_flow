...
class ArticleType(models.Model):
    ...
    Only the `name` field is stored.
    Editorial Manager and Super Admin can create, update, and delete.
    """

    # Required field
    name = models.CharField(max_length=255, unique=True)

    # Whether authors can see this article type
    is_active = models.BooleanField(default=True)
...
class Submission(models.Model):
    ...
    def sections(self):
        return {
            ...
            'submission_files': bool(self.manuscript_file),
            'title_abstract_keywords': bool(
                self.title and self.abstract and self.keywords
            ),
            'author_details': bool(self.author.full_name),
    ...
    def is_ready_to_submit(self):
        return self.completed_sections == self.total_sections and self.ethics_accepted
